...
export const createBooking = catchAsync(async (req, res) => {
  ...
  if (typeof pickupLocation !== 'object' || !pickupLocation.city) {
   ...
   if (typeof dropLocation !== 'object' || !dropLocation.city) {
      throw new BadRequestError('Invalid dropLocation object. "city" is required.');
  }
  ...
  if (user?.deviceInfo?.length > 0) {
    ...
    if (fcmToken) {
      sendBookingNotification(
        ...
        `Your booking ${booking.bookingId} from ${booking.pickupLocation.city} to ${booking.dropLocation.city} is confirmed for ${tripDate.toLocaleDateString()}.`
      ).catch(error => {
...
export const applyDiscount = catchAsync(async (req, res) => {
  ...
  return sendSuccess(
      ...
      {
          bookingId: booking.bookingId,
          fareDetails: booking.fareDetails.toObject(), // Send updated fare details
...
export const getFareEstimate = catchAsync(async (req, res) => {
  ...
  try {
      switch (type) {
       ...
       if (pricingError instanceof BadRequestError || pricingError instanceof ValidationError) {
  ...
  // --- Validate Status Transition ---
   ...
   const allowedTransitions = {
       ...
       [BOOKING_STATUS.PENDING]: {
            ...
            [BOOKING_STATUS.CANCELLED]: ['ADMIN', 'USER'] // User via cancelBooking
       },
       [BOOKING_STATUS.CONFIRMED]: {
       ...
       [BOOKING_STATUS.ASSIGNED]: {
            ...
            [BOOKING_STATUS.CANCELLED]: ['ADMIN', 'USER', 'DRIVER'] // Driver can cancel if needed? Requires reason.
       },
